...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: authData, error: authError } = await supabase.auth.signUp({
        ...
        options: {
            data: {
                ...
                // Guardamos en metadata por las dudas
      ...
      // 2. ACTUALIZACIÓN CLAVE: Aquí es donde se perdía el teléfono
      // El Trigger ya creó la fila en 'profiles', nosotros llenamos los datos manuales
      await supabase.from('profiles').update({
        first_name: formData.firstName,
        last_name: formData.lastName,
        phone: formData.phone // <--- ESTO AHORA SÍ SE GUARDA
      }).eq('id', authData.user.id);

      // OPCIONAL: Si querés que el banner de admin también se limpie rápido
