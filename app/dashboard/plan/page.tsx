...
import { 
  Loader2, Zap, User, Check, CreditCard, Clock, X, HelpCircle, 
...
function PlanContent() {
    ...
    const [restaurant, setRestaurant] = useState<any>({ business_hours: {}, subscription_plan: null, subscription_status: 'trialing' });
...
const handleTogglePause = async () => {
    const isPausing = restaurant?.subscription_status === 'authorized' || restaurant?.subscription_status === 'active';
    ...
    try {
        ...
        if (response.ok) {
            toast.success(`Suscripción ${isPausing ? 'pausada' : 'reanudada'} correctamente.`);
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                if (restData) setRestaurant({ ...restData, business_hours: restData.business_hours || {} });
            } finally {
    ...
    // --- FUNCIONES (MISMA LÓGICA QUE SETTINGS) ---
    const updateProfile = (field: string, value: string) => {
        const newData = { ...profile, [field]: value };
        setProfile(newData);
        if (saveTimeout) clearTimeout(saveTimeout);
        const newTimeout = setTimeout(async () => {
            await supabase.from('profiles').upsert({ id: userId, ...newData });
...
const handleOpenPaymentForm = (planType: string) => {
    ...
    const mountCardBrick = async (planType: string) => {
        ...
        const settings = {
            initialization: {
                amount: planType === 'light' ? 10000 : planType === 'go' ? 16900 : 27000,
                ...
                onSubmit: (formData: any) => {
                    return new Promise(async (resolve, reject) => {
                        const res = await fetch('/api/mercadopago/subscription', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ token: formData.token, payment_method_id: formData.payment_method_id, planType, userId, email: profile.email })
                        });
                        if (res.ok) {
...
const renderPlanButton = (planId: 'light' | 'go' | 'plus') => {
    ...
    const colorClass = planId === 'light' ? 'bg-black' : planId === 'go' ? 'bg-blue-600' : 'bg-emerald-600';
        ...
        return (
            <div className="flex flex-col gap-2 w-full">
                ...
                <div className="bg-emerald-100 p-2.5 rounded-xl border border-emerald-200 text-center">
                    <span className="text-[9px] font-black uppercase text-emerald-700">Plan Seleccionado</span>
                </div>
    ...
    return (
        ...
        >
            {processingPlan === planId ? (
                ...
                restaurant.subscription_plan ? 'Cambiar a este plan' : 'Activar Plan'
            )}
        </button>
    );
};
    ...
    <div>
        <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="font-bold text-lg text-gray-900 uppercase italic tracking-tighter">Método de Pago</h2>
        </div>
        
        {restaurant?.subscription_status === 'authorized' || restaurant?.subscription_status === 'active' || restaurant?.subscription_status === 'paused' ? (
            <div className="space-y-3">
                <div className={`p-3 border rounded-xl flex items-center justify-between ${restaurant.subscription_status === 'paused' ? 'bg-amber-50 border-amber-100' : 'bg-emerald-50 border-emerald-100'}`}>
                    <div className="flex items-center gap-2">
                        {restaurant.subscription_status === 'paused' ? <Clock className="text-amber-600" size={16} /> : <Check className="text-emerald-600" size={16} />}
                        <span className={`text-[9px] font-black uppercase ${restaurant.subscription_status === 'paused' ? 'text-amber-800' : 'text-emerald-800'}`}>
                            {restaurant.subscription_status === 'paused' ? 'Cobros Pausados' : 'Tarjeta Vinculada'}
                        </span>
                    </div>
                    <button onClick={() => handleOpenPaymentForm(restaurant.subscription_plan)} className="text-[9px] font-black uppercase text-gray-400 hover:text-black">Cambiar</button>
                </div>
                ...
                >
                    {restaurant.subscription_status === 'paused' ? '▶️ Reanudar Cobros y Menú' : '⏸️ Pausar Cobros Temporalmente'}
                </button>
            </div>
        ) : (
            <div className="p-4 border-2 border-dashed border-gray-100 rounded-2xl text-center">
                <p className="text-[9px] font-bold text-gray-400 uppercase mb-3">Sin tarjeta cargada</p>
                <button onClick={() => handleOpenPaymentForm(restaurant?.subscription_plan || 'light')}className="text-[9px] font-black uppercase text-blue-600 hover:underline">Vincular Ahora</button>
            </div>
        )}
    </div>
</section>
            ...
            <section className="pt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 items-start">
                    ...
                    <div className="p-6 rounded-[2rem] border-2 border-dashed border-gray-100 bg-gray-50/50 flex flex-col relative overflow-hidden transition hover:shadow-2xl h-full">
                        <div className="absolute top-3 -right-8 bg-gray-100 text-gray-400 text-[7px] font-black px-10 py-1 rotate-45 uppercase tracking-widest border-b border-gray-200">Próximamente</div>
                        ...
                        <ul className="space-y-2 mb-4 text-left">
                            ...
                            <li className="flex gap-2 text-[10px] font-bold text-gray-500"><Check size={12} className="text-purple-400 shrink-0"/> Billeteras Virtuales</li>
                        </ul>
            ...
            {showCardForm && (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                    <div className="bg-white rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl relative">
                        <button onClick={() => {setShowCardForm(false); setProcessingPlan(null);}} className="absolute top-6 right-6 text-gray-400 hover:text-black"><X size={24} /></button>
                        <div className="text-left mb-6">
                            <h3 className="text-xl font-black uppercase italic tracking-tighter">Vincular Tarjeta</h3>
                            <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">Pago seguro vía Mercado Pago</p>
                        </div>
...
export default function PlanPage() {
    return <Suspense fallback={null}><PlanContent /></Suspense>;
}