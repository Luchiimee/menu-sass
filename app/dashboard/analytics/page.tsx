...
import { 
    ...
    Calendar as CalendarIcon, ChevronLeft, ChevronRight, CheckCircle2, AlertCircle
} from 'lucide-react';
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-12 pt-6 px-4 font-sans">
        
        {/* BARRA DE NAVEGACIÓN DE FECHA (CON CALENDARIO DD/MM/AAAA) */}
        <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-3xl border shadow-sm gap-4">
            <div className="flex items-center gap-4">
                ...
                {/* Gatillo del calendario al tocar icono o fecha */}
                <div 
                    className="flex items-center gap-2 font-bold text-gray-800 cursor-pointer hover:bg-gray-50 px-3 py-1 rounded-xl transition-all relative"
        ...
        {/* LISTA DE MOVIMIENTOS DETALLADA (HISTORIAL RESTAURADO) */}
                ...
                <div className="bg-white w-full max-w-sm rounded-[32px] p-8 shadow-2xl animate-in zoom-in-95">
                    ...
                    <div className="relative mb-6">
                        ...
                        <input 
                            ...
                            className="w-full pl-10 pr-4 py-5 bg-gray-50 rounded-2xl text-4xl font-black outline-none focus:ring-4 ring-orange-100 border-none transition-all" 
                            placeholder="0" 
                            autoFocus
                        />
                    </div>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input 
                                ...
                                onChange={(e)=>setSaleData({...saleData, concept: e.target.value})} 
                                className="w-full p-4 bg-gray-50 border-none rounded-2xl font-bold outline-none focus:ring-2 ring-violet-500 transition-all" 
                                placeholder="Ej: 2 Burgers con Papas"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input 
                                    ...
                                    onChange={(e)=>setSaleData({...saleData, total: e.target.value})} 
                                    className="w-full pl-8 pr-4 py-4 bg-gray-50 border-none rounded-2xl text-2xl font-black outline-none focus:ring-2 ring-violet-500 transition-all" 
                                    placeholder="0"
                                />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <button 
                                    onClick={()=>setSaleData({...saleData, method:'efectivo'})} 
                                    className={`py-3 rounded-xl border-2 font-bold text-xs transition-all ${saleData.method === 'efectivo' ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-100 text-gray-400'}`}
                                >
                                    EFECTIVO
                                </button>
                                <button 
                                    onClick={()=>setSaleData({...saleData, method:'transferencia'})} 
                                    className={`py-3 rounded-xl border-2 font-bold text-xs transition-all ${saleData.method === 'transferencia' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-100 text-gray-400'}`}
                                >
                                    TRANSFERENCIA
                                </button>
                            </div>
