...
pop: { 
    ...
    desc: '#444444',       // DescripciÃ³n local
    card_name: '#FF1493',  // Nombre del producto en rosa
    card_desc: '#444444', 
    card_price: '#000000', // Fondo negro de la etiqueta de precio
    card_shadow_color: '#000000', // Bordes negros rÃ­gidos
    btn_bg: '#ffffff',     // Fondo del botÃ³n agregar
    btn_text: '#FF1493',   // Texto del botÃ³n agregar
  ...
  elegant: { 
    theme: '#D4AF37',        // Dorado caracterÃ­stico
    bg: '#f9f5f0',           // Crema suave de fondo
    text: '#333333',         // Texto oscuro para el nombre
    desc: '#777777',         // Gris para la descripciÃ³n
    card_name: '#333333',    // Nombre del producto
    card_color: '#f9f5f0',   // Mismo crema para que se funda
    card_desc: '#888888',    // DescripciÃ³n del producto
    card_price: '#D4AF37',   // Precio en dorado
    btn_bg: '#D4AF37',       // BotÃ³n + en dorado
    btn_text: '#ffffff',     // Cruz del botÃ³n en blanco
    promo_bg: '#f0e8dc',     // Crema mÃ¡s oscuro para la promo
    promo_text: '#5c4b30',   // Texto marrÃ³n oscuro para promo
    banner: false 
  },
  bistro: { 
    theme: '#e6c87e', 
  ...
  marketpro: { 
    ...
    promo_text: '#000000', 
    banner: true,
    // --- ESTO ES LO QUE LE FALTABA A LA GALERÃA ---
    cat_bg_color: '#f3f4f6',
...
// â”€â”€â”€ CSS del marco del selector (sin CSS de templates â€” ya son self-contained) â”€
const SELECTOR_CSS = `
  ...
  .templates-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; padding: 10px 0; align-items: start; width: 100%; }
  ...
  .phone-preview { width: 100%; max-width: 260px; height: 180px; margin: 0 auto; background: white; position: relative; overflow: hidden; border-radius: 30px; border: 1px solid rgba(0,0,0,0.1); box-shadow: 0 10px 30px -10px rgba(0,0,0,0.1); }
...
// â”€â”€â”€ MOCK DATA PARA PREVIEWS DEL SELECTOR (no afecta menÃºs reales) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const PREVIEW_NOOP = () => {};

const PREVIEW_MOCK: Record<string, any> = {
  classic: {
    restaurant: { name: 'PizzerÃ­a Los TÃ­os', description: 'Pizza a la piedra', theme_color: '#d32f2f', bg_color: '#f8f9fa', text_color: '#ffffff', show_promo: true, promo_message: 'ðŸ›µ EnvÃ­o GRATIS primera compra' },
    products: [
      { id: 'c1', name: 'Muzzarella', description: 'Salsa, muzza y orÃ©gano.', price: 8500, category_id: 'pc1' },
      { id: 'c2', name: 'Napolitana', description: 'Con ajo y perejil.', price: 10200, category_id: 'pc1' },
      { id: 'c3', name: 'FainÃ¡', description: 'PorciÃ³n individual.', price: 1500, category_id: 'pc2' },
      { id: 'c4', name: 'Coca Cola 1.5L', price: 3000, category_id: 'pc2' },
    ],
    categories: [{ id: 'pc1', name: 'Pizzas' }, { id: 'pc2', name: 'Extras' }],
  },
  urban: {
    restaurant: { name: 'Burger Joint', description: 'La mejor hamburguesa', theme_color: '#ea580c', bg_color: '#121212', text_color: '#ffffff', show_promo: true, promo_message: 'ðŸ”¥ PROMO: EnvÃ­o gratis > $15.000' },
    products: [
      { id: 'u1', name: 'Doble Black', description: 'MedallÃ³n de carne 180g.', price: 8500, image_url: 'https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=300&q=80', category_id: 'pu1' },
  ...
  minimal: {
    restaurant: { name: 'CAFÃ‰ CENTRAL', description: 'Specialty Coffee', theme_color: '#000000', bg_color: '#ffffff', text_color: '#111111', show_promo: true, promo_message: 'TAKE AWAY: 10% OFF EFECTIVO' },
    products: [
      ...
      { id: 'm3', name: 'Croissant', description: 'JamÃ³n y queso.', price: 1800, category_id: 'pm1' },
    ],
  ...
  spotlight: {
    restaurant: { name: 'Club Mercedes', description: 'Restaurante', theme_color: '#FFD700', bg_color: '#ffffff', text_color: '#000000', show_banner: true, show_promo: true, hero_title: 'Estofado de Chef', hero_price: 28000, hero_badge_text: 'PLATO DEL DÃA', hero_badge_bg: '#FFD700', hero_badge_color: '#000000', banner_url: 'https://images.unsplash.com/photo-1547592166-23ac45744acd?w=600&q=80', promo_message: 'EnvÃ­os gratis todos los jueves' },
    products: [
      ...
      { id: 's2', name: 'Matambre Pizza', description: 'Muzzarella y jamÃ³n.', price: 25000, image_url: 'https://images.unsplash.com/photo-1593560708920-61dd98c46a4e?w=300&q=80', category_id: 'ps1' },
    ],
    categories: [{ id: 'ps1', name: 'Platos' }],
  },
  icecream: {
    restaurant: { name: 'Frozen Dreams', description: 'HeladerÃ­a Artesanal', theme_color: '#06b6d4', bg_color: '#f0faff', show_promo: true, promo_message: 'ðŸ¦ PROMO: 1/4kg de regalo comprando 1kg' },
    products: [
      ...
      { id: 'h2', name: 'Granizado', description: 'LimÃ³n, menta o naranja.', price: 4500 },
    ],
  },
  marketpro: {
  ...
  'alterna-pro': {
    restaurant: { name: 'Eco Nature', description: 'Productos OrgÃ¡nicos', theme_color: '#ea580c', bg_color: '#fafaf9', text_color: '#111827', description_color: '#94a3b8', card_name_color: '#111827', card_price_color: '#ea580c', show_promo: true, promo_message: 'EnvÃ­o Gratis â€” Compras +$20.000', categories: [{ id: 'pa1', name: 'Mixes' }, { id: 'pa2', name: 'Mieles' }, { id: 'pa3', name: 'Harinas' }] },
    products: [
      ...
      { id: 'a2', name: 'Miel OrgÃ¡nica', price: 4200, image_url: 'https://images.unsplash.com/photo-1587049352846-4a222e784d38?w=300&q=80', category_id: 'pa2' },
...
// --- DATA ---
const TEMPLATES = [
  { id: 'classic',     name: 'Classic Delivery', type: 'classic',     category: 'basicas',   sale_type: 'unidad',           hasPhotos: false, businessCategory: 'delivery',     idealFor: 'Ideal para pizzerÃ­a y delivery' },
  { id: 'urban',       name: 'Urbano Dark',       type: 'urban',       category: 'basicas',   sale_type: 'unidad',           hasPhotos: true,  businessCategory: 'delivery',     idealFor: 'Ideal para hamburgueserÃ­a y street food' },
  { id: 'minimal',     name: 'Minimalista',       type: 'minimal',     category: 'basicas',   sale_type: 'unidad',           hasPhotos: false, businessCategory: 'elegante',     idealFor: 'Ideal para cafeterÃ­a y panaderÃ­a' },
  { id: 'visualgrid',  name: 'Visual Grid',       type: 'visualgrid',  category: 'basicas',   sale_type: 'unidad',           hasPhotos: true,  premium: true, businessCategory: 'heladeria',    idealFor: 'Ideal para heladerÃ­a y negocios visuales' },
  { id: 'spotlight',   name: 'Spotlight Hero',    type: 'spotlight',   category: 'basicas',   sale_type: 'unidad',           hasPhotos: true,  premium: true, businessCategory: 'restaurante',  idealFor: 'Ideal para restaurante con menÃº destacado' },
  { id: 'icecream-v1', name: 'Soft Premium',      type: 'icecream',    category: 'basicas',   sale_type: 'peso',             hasPhotos: false, premium: true, businessCategory: 'heladeria',    idealFor: 'Ideal para heladerÃ­a y dietÃ©tica' },
  ...
  { id: 'carta',       name: 'Carta',             type: 'carta',       category: 'completas', sale_type: 'unidad',           hasPhotos: true,  premium: true, businessCategory: 'restaurante',  idealFor: 'Ideal para restaurante con menÃº destacado' },
];
...
useEffect(() => {
  ...
  // --- LOGICA DE SELECCIÃ“N CORREGIDA (HARD RESET) ---
 const handleSelect = async (id: string, premium: boolean) => {
    if (premium && userPlan === 'free') return alert("Esta es una plantilla Premium. ActualizÃ¡ tu plan para usarla.");
    ...
    if (user) {
      ...
      // HARD RESET: Mandamos todos los colores por defecto de la galerÃ­a a la DB
    // HARD RESET: Mandamos todos los colores por defecto de la galerÃ­a a la DB
      await supabase.from('restaurants').update({ 
          ...
          // --- AQUÃ LA GALERÃA GUARDA LOS COLORES DE MARKETPRO ---
      ...
      toast.success(`Â¡DiseÃ±o ${id.toUpperCase()} activado con sus colores originales!`);
    }
    setSavingId(null);
  };
    ...
    <div className="relative px-4 pt-0 lg:px-8 min-h-[85vh] bg-gray-50/50 pb-20">
      ...
      <header className="mb-10 pt-20 lg:pt-0 text-left relative z-10">
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl sm:text-4xl font-black text-gray-900 uppercase italic tracking-tighter leading-none">
            GalerÃ­a de DiseÃ±os
          </h1>
          <p className="text-gray-500 text-xs sm:text-sm font-medium leading-tight">
            Elige el diseÃ±o perfecto para el link digital de tu negocio
          </p>
        </div>
      </header>
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 mb-10">
        {([
          ...
          { key: 'restaurante', label: 'ðŸ½ï¸ Restaurante con menÃº destacado' },
          { key: 'delivery',    label: 'ðŸ” Delivery y comida rÃ¡pida' },
          { key: 'kiosco',      label: 'ðŸ›ï¸ Kiosco y despensa' },
          { key: 'heladeria',   label: 'ðŸ¦ HeladerÃ­a y dietÃ©tica' },
          { key: 'elegante',    label: 'âœ¨ Elegante y minimalista' },
...
{/* GRILLA DE MOCKUPS â€” CartProvider necesario para los 5 templates que usan useCart() */}
      <CartProvider>
      <div className="templates-grid animate-in fade-in duration-500 pb-20">
        {finalTemplates.map((t) => {
          ...
          // Escalas confirmadas visualmente â€” cada una muestra header + al menos 1 elemento de contenido
          const desktopScale =
            t.id === 'classic'     ? 0.68 :
            t.id === 'urban'       ? 0.72 :
            t.id === 'minimal'     ? 0.68 :
  ...
  <div className="template-card">
    <div className="phone-preview">
      <div className="preview-content">
        <div style={{
          width: `${Math.round(100 / previewScale)}%`,
        ...
        {/* Store icon overlay â€” nunca en la misma esquina que el badge nativo del componente */}
        {t.id === 'carta' ? (
          // Carta: badge nativo top-left â†’ Store top-right
        ...
        ) : t.id === 'spotlight' || t.id === 'alterna-pro' ? (
          // Spotlight y Alterna Pro: tienen su propio Store icon en el header â€” omitir overlay
          null
        ) : (
          // classic, urban, minimal, visualgrid, icecream, marketpro:
          // badge nativo top-right â†’ Store top-left
   ...
   <div className="card-info text-center flex flex-col items-center">
      {(t as any).featured && (
        <span className="mt-2 px-2.5 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[9px] font-black uppercase tracking-widest border border-amber-200">
          â­ MÃ¡s elegida
        </span>
      )}
      <h3 className="card-title mt-1 text-slate-800">{t.name}</h3>
      ...
      {isUpcoming ? (
        ...
        <div className="flex flex-col items-center gap-2 mt-2 w-full px-2 animate-in fade-in zoom-in-95 duration-300">
           <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest flex items-center gap-1">
             <Check size={10} strokeWidth={4}/> DiseÃ±o en uso
           </span>
           <Link
        ...
        <button
          ...
          className="mt-3 w-full px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm disabled:opacity-50 flex items-center justify-center gap-1.5 active:scale-95"
        >
          {savingId === t.id ? (
  ...
  return (
    <Suspense fallback={
      <div className="fixed inset-0 flex flex-col items-center justify-center bg-white z-[300]">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 animate-pulse">Preparando diseÃ±os...</p>
      </div>
    }>
      <GalleryContent />
    </Suspense>
