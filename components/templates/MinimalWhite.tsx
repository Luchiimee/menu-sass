...
import { Coffee, Search, Layers, Plus, Minus, Check, Store, X, MapPin, Phone, Instagram, Facebook } from 'lucide-react';
...
const MinimalWhite: React.FC<Props> = ({ 
  ...
  const headerBg = restaurant.theme_color || '#111111';
...
const prodDesc = restaurant.card_desc_color || '#555555';
  ...
  const styles = `
    ...
    .minimal-logo { 
      ...
      border: 4px solid white; /* Borde más marcado */
  ...
  .minimal-status { 
      ...
      padding: 6px 14px; /* Más cuerpo */
      border-radius: 8px; 
      letter-spacing: 1px; 
      border: 1px solid ${isOpen ? '#eee' : '#fee2e2'}; 
      background: ${isOpen ? '#fff' : '#fef2f2'}; 
      color: ${isOpen ? '#111' : '#ef4444'}; 
...
.minimal-prod-desc { 
      font-size: 12px; /* Subió de 9px a 12px */
      color: ${prodDesc}; 
      margin-top: 4px; 
      opacity: 0.9; /* Subió de 0.7 a 0.9 para que no sea tan claro */
      line-height: 1.4; 
    }
  ...
  className="minimal-info-btn" 
  style={{ color: restaurant.theme_color || '#111111' }}
>
  <Store size={24}/>
</button>

        <div className="minimal-logo" style={restaurant.logo_url ? { backgroundImage: `url('${restaurant.logo_url}')`, color: 'transparent' } : {}} >
          {!restaurant.logo_url && <Coffee size={32}/>} {/* Icono fallback más grande */}
        </div>
        ...
        {/* 🚀 DESCRIPCIÓN: Subimos tamaño a 13px y opacidad a 0.8 para que se lea perfecto */}
    ...
    {p.description}
                        ...
                        <div style={{ fontWeight: 900, fontSize: '12px', color: priceColor, marginTop: '6px' }}>
    {formatPrice(p.price)}
  </div>
</div>
                      {cartItem ? (
