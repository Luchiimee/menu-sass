      ...
      {showInfo && (
        <div className="fixed inset-0 z-[300] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="w-full max-w-sm bg-white rounded-[2.5rem] p-8 animate-in zoom-in-95 duration-300 shadow-2xl relative">
            ...
            <div className="space-y-8">
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  {restaurant.google_maps_link ? (
                    ...
                    <p className="text-sm font-bold text-gray-800">{restaurant.address || 'Consultar dirección por WhatsApp'}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-gray-800 whitespace-pre-line leading-snug">{restaurant.opening_hours || 'No especificados'}</p>
                </div>
              </div>

              {/* 2.5 WHATSAPP DE CONTACTO */}
              {restaurant.phone && (
                <div className="flex gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">WhatsApp Directo</p>
                    <a 
                      href={`https://wa.me/${restaurant.phone.replace(/\D/g, '')}`} 
                      target="_blank" rel="noopener noreferrer"
                      className="text-sm font-bold text-green-600 underline decoration-green-200 underline-offset-4 flex items-center gap-1 active:opacity-70 transition-all"
                    >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-3">
                      {restaurant.instagram && (
                        <a href={restaurant.instagram.startsWith('http') ? restaurant.instagram : `https://${restaurant.instagram}`} target="_blank" rel="noopener noreferrer" className="p-2.5 bg-pink-50 rounded-xl hover:scale-110 transition-transform">
                          <Instagram size={20} className="text-pink-600"/>
                        </a>
                      )}
                      {restaurant.facebook && (
                        <a href={restaurant.facebook.startsWith('http') ? restaurant.facebook : `https://${restaurant.facebook}`} target="_blank" rel="noopener noreferrer" className="p-2.5 bg-blue-50 rounded-xl hover:scale-110 transition-transform">
                          <Facebook size={20} className="text-blue-600"/>
                        </a>
                      )}
                      {restaurant.tiktok && (
                        <a href={restaurant.tiktok.startsWith('http') ? restaurant.tiktok : `https://${restaurant.tiktok}`} target="_blank" rel="noopener noreferrer" className="p-2.5 bg-zinc-100 rounded-xl hover:scale-110 transition-transform">
            ...
            <button onClick={() => setShowInfo(false)} className="w-full mt-10 py-4 bg-black text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all">
              Entendido
            </button>
          </div>
        </div>
