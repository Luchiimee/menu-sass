...
export default function VisualGrid({ restaurant, products, categories, fetchedExtras, isOpen, setShowInfo, onAddToCart, isMockup, mesaLabel = null }: any) {
  ...
  const catInactiveBorder = getContrastColor(BG) === '#000000' ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.3)';
  ...
  return (
    <div style={{ background: BG, minHeight: '100vh', display: 'flex', flexDirection: 'column', padding: isMockup ? '8px' : '12px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 10, overflowX: 'auto', padding: '0 4px 20px' }} className="no-scrollbar">
        <button onClick={() => setSelectedCategory("todos")} style={{ background: selectedCategory === "todos" ? (restaurant?.cat_active_bg_color || ACENTO) : 'transparent', color: selectedCategory === "todos" ? (restaurant?.cat_active_text_color || '#ffffff') : (restaurant?.cat_text_color || '#ffffff'), border: `1px solid ${selectedCategory === "todos" ? (restaurant?.cat_active_bg_color || ACENTO) : catInactiveBorder}`, padding: '10px 20px', borderRadius: '15px', fontSize: '10px', fontWeight: '900', textTransform: 'uppercase', whiteSpace: 'nowrap', transition: 'all 0.2s' }}>
          Todos
        </button>
        {categories?.filter((c: any) => c.name.toLowerCase() !== 'general').map((cat: any) => (
          <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} style={{ background: selectedCategory === cat.id ? (restaurant?.cat_active_bg_color || ACENTO) : 'transparent', color: selectedCategory === cat.id ? (restaurant?.cat_active_text_color || '#ffffff') : (restaurant?.cat_text_color || '#ffffff'), border: `1px solid ${selectedCategory === cat.id ? (restaurant?.cat_active_bg_color || ACENTO) : catInactiveBorder}`, padding: '10px 20px', borderRadius: '15px', fontSize: '10px', fontWeight: '800', textTransform: 'uppercase', whiteSpace: 'nowrap', transition: 'all 0.2s' }}>
            {cat.name}
          </button>
        ))}
      </div>
